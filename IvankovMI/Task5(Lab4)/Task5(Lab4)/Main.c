...
.Galya / .remove         — отменить уже добавленный к покупке товар \n\
.Galina / .dropbasket    — отменить весь процесс покупки \n\
...
void Galya(char* inp, struct item_in_receipt receipt[], int n) {
	...
	if (product_ptr)
		if (all) {
			...
			(product_ptr->count)--;
			printf("Колличество данного товара уменьшино на 1\n");
		}
	else
		printf("Товар с таким штрихкодом не найден в корзине.\n");
}
...
void Galina(char* inp, int* coup, struct item_in_receipt receipt[], int* n) {
	...
	printf("Вы добавили в корзину покупки. Дождитесь сотрудника, чтобы он ввел\nпароль: ");
	scan;
	if (!(R(STFPASS))) {
		printf("Пароль неверный. Чтобы отменить покупку введите команду еще раз.\n");
		return;
	}
	*coup = 0;
	*n = 0;
...
void final(char* inp, int* coup, char coupon[], struct item_in_receipt basket[], int* n, int interactive, int* cli, int* tot) {
	...
	FILE* file = NULL;
	if (*n == 0) {
		printf("Вы не добавили ни одного товара. Вводите их штрихкоды для добавления в корзину.\n");
		return;
	}
	if (! *coup) {
		if (is_last_word()) {
